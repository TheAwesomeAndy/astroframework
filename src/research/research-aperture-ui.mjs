...
function applyAperture(){const research=aperture==='research';document.getElementById('personalMode')?.classList.toggle('active',!research);document.getElementById('researchMode')?.classList.toggle('active',research);document.querySelectorAll('#networkContent .finding').forEach(el=>{if(el.querySelector('.tag.discovery'))el.style.display=research?'':'none';});document.querySelectorAll('#networkContent [data-aspect-mode="extended"]').forEach(el=>el.style.display=research?'':'none');document.querySelectorAll('#readingContent .panel').forEach(p=>{if(p.querySelector('h2')?.textContent?.trim().toLowerCase()==='discoveries')p.style.display=research?'':'none';});renderPanel();}
export function setAperture(mode){aperture=mode==='research'?'research':'personal';applyAperture();}
function rebuild(){ensureControls();buildApertureLab(window.NoeticAtlasState);applyAperture();}
const css=make('style');css.textContent='.research-aperture{display:flex;gap:4px;margin-left:auto}.research-aperture button.active{background:#70b8ba;color:#111;border-color:#70b8ba}.research-regime-card,.research-candidate{border-top:1px solid #25313b;padding:7px 0}';document.head.appendChild(css);ensureControls();window.addEventListener('noetic:state',rebuild);const observer=new MutationObserver(applyAperture);for(const id of ['networkContent','readingContent']){const el=document.getElementById(id);if(el)observer.observe(el,{childList:true,subtree:true});}rebuild();
