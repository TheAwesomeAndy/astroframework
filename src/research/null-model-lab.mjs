...
import {NULL_METRICS,metricDefinition,buildObservedMetricVector} from './null-metric-registry.mjs';
...
export function runNullTest({analysis,graph=null,river=null,discoveries=null,target_id='chart',model_id,metric_id,iterations=512,seed='naf-v046',observed_metrics=null}={}){
  ...
  const metric=metricDefinition(metric_id),obs=observed_metrics||buildObservedMetricVector({graph,river,discoveries}),observed=Number(obs[metric_id]);if(!Number.isFinite(observed))throw new Error(`Observed metric unavailable: ${metric_id}`);
