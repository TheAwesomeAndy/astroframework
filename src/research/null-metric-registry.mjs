...
export function basinMetricsFromCounts(counts){
  const ps=normalizeCounts(counts),entropy=ps.length? -ps.reduce((s,p)=>s+p*log2(p),0):0,concentration=ps.reduce((s,p)=>s+p*p,0),largest=ps.length?Math.max(...ps):0;
  return {house_basin_entropy:round(entropy),house_basin_concentration:round(concentration),largest_house_basin_fraction:round(largest)};
}

export function countTriangles(nodes,edges){
  const adj=Object.fromEntries(nodes.map(n=>[n,new Set()]));
  for(const [a,b] of edges){if(adj[a]&&adj[b]){adj[a].add(b);adj[b].add(a);}}
  let count=0;const ns=[...nodes].sort();
  for(let i=0;i<ns.length;i++)for(let j=i+1;j<ns.length;j++)for(let k=j+1;k<ns.length;k++)if(adj[ns[i]].has(ns[j])&&adj[ns[i]].has(ns[k])&&adj[ns[j]].has(ns[k]))count++;
  return count;
}

export function countArticulationPoints(nodes,edges){
  const adj=Object.fromEntries(nodes.map(n=>[n,new Set()]));for(const [a,b] of edges){if(adj[a]&&adj[b]){adj[a].add(b);adj[b].add(a);}}
  let time=0;const disc={},low={},parent={},arts=new Set();
  const dfs=u=>{disc[u]=low[u]=++time;let children=0;for(const v of adj[u]){if(!disc[v]){children++;parent[v]=u;dfs(v);low[u]=Math.min(low[u],low[v]);if(parent[u]===undefined&&children>1)arts.add(u);if(parent[u]!==undefined&&low[v]>=disc[u])arts.add(u);}else if(v!==parent[u])low[u]=Math.min(low[u],disc[v]);}};
  for(const n of nodes)if(!disc[n])dfs(n);return arts.size;
}

export function buildObservedMetricVector({graph=null,river=null,discoveries=null,house_terminal_counts=null}={}){
  let counts=house_terminal_counts;
  if(!counts){
    const finding=(discoveries?.findings||[]).find(f=>f.kind==='house_terminal_partition');
    if(finding?.measurement?.partitions)counts=Object.fromEntries(finding.measurement.partitions.map(p=>[String(p.terminal),Number(p.house_count)]));
  }
  const basin=basinMetricsFromCounts(counts||{}),aspect=graph?.graphs?.aspect||{},classical=graph?.graphs?.classical_dispositor||{};
  const edges=(aspect.edges||[]).map(e=>[e.a||e.from,e.b||e.to]).filter(e=>e[0]&&e[1]);
  const nodes=aspect.nodes||[...new Set(edges.flat())];
  const multi=(discoveries?.findings||[]).filter(f=>f.kind==='multi_role_planet').map(f=>Number(f.measurement?.role_count||0));
  return {
    ...basin,
    terminal_basin_count:(classical.terminal_basins||[]).length,
    max_route_capture:Number(river?.max_route_count||0),
    aspect_triangle_count:countTriangles(nodes,edges),
    aspect_articulation_count:Array.isArray(aspect.articulation_points)?aspect.articulation_points.length:countArticulationPoints(nodes,edges),
