...
assert.doesNotMatch(html,/data-view="research"/,'Research must be an aperture control, not an eighth top-level view');
...
assert.match(html,/registered-not-executed/);assert.match(html,/Research constitution/);
assert.match(html,/A delta is not evidence that the experimental model is superior/);
assert.doesNotMatch(html,/src="\.\/v04\d/,'candidate must not embed historical version shells');
